...
void dnuma_move_to_new_ml(struct memlayout *new_ml)
{
	...
	ml_for_each_range(new_ml, rme) {
		...
		for (pfn = rme->pfn_start; pfn <= rme->pfn_end; pfn++) {
			...
			/* FIXME: split pages when the range does not cover an entire order */
			...
			if (WARN(last_pfn > rme->pfn_end, "last_pfn %lu goes beyond end of rme [ %lu - %lu ]\n",
...
skip_unlock:
		...
		list_for_each_entry_safe(page, page_tmp, &list, lru) {
			...
			//grow_pgdat_and_zone(zone, pfn, pfn + 1 << order);
			dnuma_prior_add_to_new_zone(page, order, zone, range_nid);

			// hits VM_BUG in "static inline void __SetPageBuddy(struct page *page)"
