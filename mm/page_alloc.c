...
static int page_outside_zone_boundaries(struct zone *zone, struct page *page)
{
	...
	if (ret)
		pr_err("page with pfn %05lx outside zone %s with pfn range [ %05lx - %05lx ] in node %d with pfn range [ %05lx - %05lx ]\n",
			pfn, zone->name, start_pfn, start_pfn + sp, zone->zone_pgdat->node_id, zone->zone_pgdat->node_start_pfn, pgdat_end_pfn(zone->zone_pgdat));
...
static unsigned nr_node_ids_mod_percent = 0;
...
static void _zone_set_pageset_highmark(void *data)
{
	...
	setup_pagelist_highmark(per_cpu_ptr(zone->pageset, smp_processor_id()), high);
}
