...
#if USE_FAILING_INCLUDE
/* avoids eventual attempt to include non-exsistent asm/bug.h */
#define _LINUX_MMZONE_H
//#define _LINUX_STDDEF_H

/* avoids attempt to include non-exsistent asm/linkage.h */
...
/* using linux/types.h causes conflicts for fd_set, mode_t, dev_t, timer_t, and blkcnt_t */
...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (cur[0] == '0' && cur[1] == 'x')
			cur = &cur[2];
		unsigned long flags = strtoul(cur, &r, 16);
		if (*r != '\0') {
