...
enum perf_group_flag {
	 ...
	 *
	 * "ALWAYS_SCHEDULABLE", "EVENT_TXN"
	 */
	PERF_GROUP_ALWAYS_SCHEDULABLE = 0x2,
	 ...
	 * resources) (PERF_GROUP_ALWAYS_SCHEDULABLE).
	 *
...
static size_t event_get_##name##_max(void)				\
{									\
	int bits = (bit_end) - (bit_start) + 1;				\
	return ((0x1ULL << ((bits * 8ULL) - 1ULL)) - 1ULL) |		\
		(0xFULL << ((bits * 8ULL) - 4ULL));			\
}									\
static u64 event_get_##name(struct perf_event *____pra___event)		\
{									\
	return (____pra___event->attr.attr_var >> (bit_start)) &	\
