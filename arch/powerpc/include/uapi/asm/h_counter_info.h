#ifndef _H_COUNTER_INFO_H_
#define _H_COUNTER_INFO_H_
...
struct phyp_perf_counter_info_params {
...
enum counter_info_requests {
	...
	/* Data from this should only be considered valid if counter_info_version >= 0x3
	 * @starting_index: starting hardware chip id or -1 for the current hw chip id
	 * @secondary_index: unused
	 */
...
struct cv_dispatch_timebase_by_processor {
	...
	__u8 reserved[0x2];
	__be32 processor_id_register; /* counter_info_version >= 0x3 || version >= 0x1 */
...
struct cv_system_performance_capabilities {
	__u8 perf_collect_privlidged; /* If != 0, allowed to collect data from other partitions */
