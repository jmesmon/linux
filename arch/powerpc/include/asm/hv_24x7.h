#ifndef ARCH_POWERPC_UAPI_24x7_H_
#define ARCH_POWERPC_UAPI_24x7_H_

#include <linux/types.h>

struct phyp_24x7_request {
	/* PHYSICAL domains require enabling via phyp/hmc. */
#define PHYP_24X7_PERF_DOMAIN_PHYSICAL_CHIP 0x01
#define PHYP_24X7_PERF_DOMAIN_PHYSICAL_CORE 0x02
#define PHYP_24X7_PERF_DOMAIN_VIRTUAL_PROCESSOR_HOME_CORE   0x03
#define PHYP_24X7_PERF_DOMAIN_VIRTUAL_PROCESSOR_HOME_CHIP   0x04
#define PHYP_24X7_PERF_DOMAIN_VIRTUAL_PROCESSOR_HOME_NODE   0x05
#define PHYP_24X7_PERF_DOMAIN_VIRTUAL_PROCESSOR_REMOTE_NODE 0x06
	__u8 performance_domain;

	__u8 reserved[0x1];
	__be16 data_size; /* bytes to read starting at @data_offset. must be a multiple of 8 bytes */
	__be32 data_offset; /* byte offset within the perf domain to read from. must be 8 byte aligned */
	/* only valid for VIRTUAL_PROCESSOR domains, ignored for others.
	 * -1 means "current partition only"
	 *  Enabling via phyp/hmc required for non-"-1" values. 0 forbidden unless requestor is 0.
	 */
...
struct phyp_24x7_request_buffer {
	/* 0 - ? */
	/* 1 - ? */
#define PHYP_24X7_IF_VERSION_CURRENT 0x01
	__u8 interface_version;
	__u8 num_requests;
	__u8 reserved[0xE];
	struct phyp_24x7_request requests[];
} __packed;

struct phyp_24x7_result_element {
	__be16 lpar_ix;

	/* represents the core, chip, or virtual processor based on the request's @performance_domain */
	__be16 domain_ix;
...
struct phyp_24x7_result {
	...
	/* This is a copy of @data_size from the coresponding phyp_24x7_request */
	__be16 result_element_data_size;
	__u8 reserved[0x2];

	/* WARNING: only valid for first result element due to variable sizes
	 *          of result elements */
	/* struct phyp_24x7_result_element[@num_elements_returned] */
	struct phyp_24x7_result_element elements[];
} __packed;

struct phyp_24x7_data_result_buffer {
	...
	__u8 reserved2[0x8];
	/* WARNING: only valid for the first result due to variable sizes of results */
	struct phyp_24x7_result results[]; /* [@num_results] */
} __packed;

/* From document "24x7 Event and Group Catalog Formats Proposal" v0.14 */
struct phyp_24x7_catalog_page_0 {
#define PHYP_24X7_CATALOG_MAGIC 0x32347837 /* "24x7" in ASCII */
	__be32 magic;
...
struct phyp_24x7_event_data {
	...
	__be16 event_counter_offs; /* in bytes, offset from event_group_record */
...
enum phyp_24x7_grs_field_enums {
	/* GRS_COUNTER_1 = 1
	 * GRS_COUNTER_2 = 2
	 * ...
...
enum phyp_24x7_grs_enums {
	GRS_CORE_SCHEMA_INDEX = 0,
};

struct phyp_24x7_grs_field {
	__be16 field_enum;
...
/* operators: '+', '-', '*', '/', 'mod', 'rem', 'sqr', 'x^y' (XXX: pow? xor?), 'rot', 'dup' */
